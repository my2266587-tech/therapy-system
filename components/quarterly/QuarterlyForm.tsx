...
interface Props { initial: QuarterlySummary | null; onSave: () => void; onCancel: () => void; }

export default function QuarterlyForm({ initial, onSave, onCancel }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = initial?.id
      ? await supabase.from('quarterly_summaries').update(payload).eq('id', initial.id)
      : await supabase.from('quarterly_summaries').insert(payload);
    setSaving(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="פגישה עם" value={form.participants} onChange={v => set('participants', v)} placeholder="שמות המשתתפים..." />
        <Field label="משך (דקות)" type="number" value={form.duration_minutes} onChange={v => set('duration_minutes', v)} />
      </div>
      <TextareaField label="סיכום" value={form.summary} onChange={v => set('summary', v)} rows={4} />
