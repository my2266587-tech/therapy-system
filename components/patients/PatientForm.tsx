...
interface Props { initial: Patient | null; onSave: () => void; onCancel: () => void; }
...
export default function PatientForm({ initial, onSave, onCancel }: Props) {
  ...
  const [coordinators,  setCoordinators]  = useState<StaffOpt[]>([]);
  const [staffMembers,  setStaffMembers]  = useState<StaffOpt[]>([]);
  ...
  useEffect(() => {
    ...
    supabase.from('staff').select('id, full_name').neq('role', 'coordinator').order('full_name')
  ...
  const coordinatorOptions = coordinators.map(s => ({ value: s.id, label: s.full_name }));
  const staffMemberOptions  = staffMembers.map(s => ({ value: s.id, label: s.full_name }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <FormGroup title="שיוך לצוות">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <SelectField label="רכזת אחראית" value={form.coordinator_id} onChange={v => set('coordinator_id', v)} options={coordinatorOptions} placeholder="בחרי רכזת..." />
          <SelectField label="איש צוות אחראי" value={form.staff_id} onChange={v => set('staff_id', v)} options={staffMemberOptions} placeholder="בחרי איש צוות..." />
        </div>
      </FormGroup>
      ...
      <div className="flex gap-3 pt-1">
        <button type="submit" disabled={saving}
          className="px-6 py-2.5 bg-teal-700 text-white text-sm font-semibold rounded-lg hover:bg-teal-800 disabled:opacity-50 transition-colors shadow-sm">
