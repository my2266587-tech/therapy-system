...
export default function PettyCashPage() {
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-6 py-8">
      <PageHeader title="מעשר געלט" description="הוצאות קטנות שוטפות"
      ...
      {records.length > 0 && (
        <div className="bg-teal-50 border border-teal-200 rounded-xl p-4 mb-6 text-sm text-teal-800">
          סה"כ: <strong>₪{total.toLocaleString('he-IL', { minimumFractionDigits: 2 })}</strong>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                {['תאריך','סכום','עבור','הערות','פעולות'].map(h => (
            ...
            <tbody className="divide-y divide-slate-100">
              {records.map(r => (
                <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-medium text-slate-700">₪{Number(r.amount).toFixed(2)}</td>
