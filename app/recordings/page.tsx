              ...
              {records.map((r, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5">
                    <span className="text-xs font-medium" style={{ color: r.transcript ? '#0F766E' : '#94A3B8' }}>
                      {r.transcript ? 'קיים' : '—'}
                    </span>
                  </td>
                  <td className="px-4 py-3.5">
                    <span className="text-xs font-medium" style={{ color: r.draft_summary ? '#0F766E' : '#94A3B8' }}>
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="bg-white rounded-xl p-16 text-center" style={{ border: '1px solid #E2E8F0' }}>
      ...
      <p className="font-semibold mb-1" style={{ color: '#0F172A' }}>אין הקלטות עדיין</p>
      <p className="text-sm mb-4" style={{ color: '#94A3B8' }}>לחצי להוספת הרשומה הראשונה</p>
      <button onClick={onAdd} className="px-4 py-2 rounded-lg text-sm font-semibold text-white" style={{ backgroundColor: '#0F766E' }}>+ הוסף רשומה</button>
    </div>
  );
}
